...
void draw()
{
  ...
  *****************************************************************************/
  ...
  if (showPressStart) {
    ...
    // Spawn asteroids for title screen
    ...
    if (blackHoleDeath) {
      ...
      // Shrink UFO size
      ufoDeathSize = 105 * (1.0 - animationProgress);
...
// Black hole force calculation - 0.4N constant force when UFO edge touches outer ring
PVector calculateBlackHoleForce(BlackHole bh, PVector ufoPos) {
  ...
  // UFO radius is 105/2 = 52.5 pixels
  float ufoRadius = 52.5;
  
  // Black hole outer ring radius is (size + 250)/2 = (149 + 250)/2 = 199.5 pixels  
  float blackHoleOuterRadius = (bh.size + 250) / 2;
  
  // Check if UFO edge is within black hole outer ring
  if (distance <= blackHoleOuterRadius + ufoRadius) {
    // Apply 0.4N force toward black hole
    PVector forceDirection = PVector.sub(bh.pos, ufoPos);
    forceDirection.normalize();
    forceDirection.mult(0.4); // 0.4N force
    
    // Convert from screen coordinates to mm coordinates
...
void drawShrinkingSpaceship(float xPos, float yPos, float size) {
  ...
  float scaleFactor = size / 105.0; // Scale based on original size
