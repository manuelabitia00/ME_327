...
float ufoDeathSize = 120; // Actual UFO size
...
int currentAvatar = 0; // Index 0-7 for base UFO + img1-img7
  ...
  *****************************************************************************/
  ...
  if (showPressStart) {
    ...
    if (blackHoleDeath) {
      ...
      // Shrink UFO size
      ufoDeathSize = 120 * (1.0 - animationProgress); // Actual UFO size
...
void checkCollisions(float xPosWindow, float yPosWindow) {
  ...
  for (BlackHole bh : blackHoles) {
    if (PVector.dist(playerPos, bh.pos) < bh.eventHorizon) {
      ...
      ufoDeathSize = 120; // Actual UFO size (was 200, now 120)
      break;
    }
  }
}
...
void createSmokeParticle() {
  ...
  // Create multiple smoke particles for more visible effect
  int numSmokeParticles = 1 + damageLevel; // More smoke with more damage
  for (int i = 0; i < numSmokeParticles; i++) {
    PVector smokeOffset = new PVector(random(-20, 20), random(-20, 20));
    smokeParticles.add(new SmokeParticle(PVector.add(smokePos, smokeOffset), smokeVel.copy()));
  }
  
  // At 2+ hearts lost (damageLevel 2+), add more fire particles
  if (damageLevel >= 2) {
    // Main fire particles - more numerous and brighter
    for (int i = 0; i < 2 + damageLevel; i++) { // More fire with more damage
      PVector firePos = new PVector(xPosWindow, yPosWindow);
      firePos.add(random(-25, 25), random(-25, 25));
      PVector fireVel = PVector.random2D();
      fireVel.mult(random(1, 4)); // Faster fire particles
      particles.add(new Particle(firePos, fireVel, color(255, random(150, 255), random(0, 80)))); // Brighter fire colors
    }
  }
  
  // At 1 heart (damageLevel 4), add massive fire and electrical effects
  if (damageLevel >= 4) {
    // Intense fire particles
    for (int i = 0; i < 5; i++) {
      PVector firePos = new PVector(xPosWindow, yPosWindow);
      firePos.add(random(-30, 30), random(-30, 30));
      PVector fireVel = PVector.random2D();
      fireVel.mult(random(2, 5));
      particles.add(new Particle(firePos, fireVel, color(255, random(200, 255), random(0, 50)))); // Very bright fire
    }
    
    // Electrical sparks
    for (int i = 0; i < 3; i++) {
      PVector sparkPos = new PVector(xPosWindow, yPosWindow);
      sparkPos.add(random(-40, 40), random(-40, 40));
      PVector sparkVel = PVector.random2D();
      sparkVel.mult(random(3, 8)); // Fast sparks
      particles.add(new Particle(sparkPos, sparkVel, color(255, 255, random(100, 255)))); // Bright yellow/white sparks
    }
    
    // Mini flames coming off the UFO - more numerous
    for (int i = 0; i < 6; i++) { // More flames (was 3, now 6)
      PVector flamePos = new PVector(xPosWindow, yPosWindow);
      float angle = random(TWO_PI);
      float dist = random(30, 55); // Slightly larger spread
      flamePos.add(cos(angle) * dist, sin(angle) * dist);
      PVector flameVel = new PVector(cos(angle), sin(angle));
      flameVel.mult(random(1.0, 2.5)); // Faster flames
      particles.add(new Particle(flamePos, flameVel, color(255, random(180, 255), random(0, 60)))); // Brighter flames
    }
  }
}
...
void keyPressed() {
  if (showPressStart) {
    if (key == ' ') {
      ...
      if (keyCode == LEFT) {
        currentAvatar = (currentAvatar - 1 + 8) % 8; // Cycle backwards through avatars
      } else if (keyCode == RIGHT) {
        currentAvatar = (currentAvatar + 1) % 8; // Cycle forwards through avatars
      }
    }
  } else if (enteringName) {
...
class SmokeParticle {
  ...
  SmokeParticle(PVector p, PVector v) {
    pos = p.copy();
    vel = v.copy();
    maxLife = life = random(50, 100); // Longer lasting (was 40-80, now 50-100)
    size = random(6, 16); // Larger smoke (was 4.5-12, now 6-16)
    
    // Smoke color varies based on damage level
    if (damageLevel >= 4) {
      smokeColor = color(60, 50, 40); // Dark brown smoke for severe damage
    } else if (damageLevel >= 2) {
      smokeColor = color(70, 60, 50); // Brown-gray smoke for moderate damage
    } else {
      smokeColor = color(80, 80, 80); // Gray smoke for light damage
    }
  }
  ...
  void draw() {
    float alpha = map(life, 0, maxLife, 0, 180); // More opaque (was 75, now 180)
...
class Particle {
  ...
  void draw() {
    float alpha = map(life, 0, maxLife, 0, 200); // More opaque (was 128, now 200)
...
void drawSpaceship(float xPosWindow, float yPosWindow) {
  ...
  if (!invincible || frameCount % 10 < 5) {
    ...
    // Damage effects - enhanced visibility
    if (damageLevel > 0) {
      // Hull cracks and damage lines
      stroke(255, 100, 100, 200);
      strokeWeight(3);
      for (int i = 0; i < damageLevel * 4; i++) { // More cracks (was 3, now 4)
        float angle = random(TWO_PI);
        float startR = spaceshipSize * 0.15; // Start closer to center
        float endR = spaceshipSize * 0.45; // Extend further out
        float x1 = cos(angle) * startR;
        float y1 = sin(angle) * startR;
        float x2 = cos(angle + random(-0.4, 0.4)) * endR; // More jagged cracks
        float y2 = sin(angle + random(-0.4, 0.4)) * endR;
        line(x1, y1, x2, y2);
      }
      
      // Additional thin damage lines
      stroke(200, 80, 80, 150);
      strokeWeight(1);
      for (int i = 0; i < damageLevel * 6; i++) {
        float angle = random(TWO_PI);
        float startR = spaceshipSize * 0.1;
        float endR = spaceshipSize * 0.35;
        float x1 = cos(angle) * startR;
        float y1 = sin(angle) * startR;
        float x2 = cos(angle + random(-0.6, 0.6)) * endR;
        float y2 = sin(angle + random(-0.6, 0.6)) * endR;
        line(x1, y1, x2, y2);
      }
      
      // Hull burn marks and chips
      fill(30, 20, 10, 200); // More opaque burn marks (was 150, now 200)
      noStroke();
      for (int i = 0; i < damageLevel * 3; i++) { // More burn marks (was 2, now 3)
        float angle = random(TWO_PI);
        float dist = random(spaceshipSize * 0.15, spaceshipSize * 0.42); // Spread further
        float x = cos(angle) * dist;
        float y = sin(angle) * dist;
        ellipse(x, y, random(8, 18), random(5, 12)); // Larger burn marks
      }
      
      // Metal chips and gouges
      ...
      for (int i = 0; i < damageLevel * 2; i++) {
        ...
        float x = cos(angle) * dist;
        float y = sin(angle) * dist;
        // Draw irregular chip shapes
        beginShape();
        for (int j = 0; j < 5; j++) {
          float chipAngle = TWO_PI * j / 5 + random(-0.3, 0.3);
          float chipR = random(3, 8);
          vertex(x + cos(chipAngle) * chipR, y + sin(chipAngle) * chipR);
        }
        endShape(CLOSE);
      }
      
      // Severe damage at very low health
      if (damageLevel >= 3) {
        // Large hull breaches
        fill(0, 0, 0, 150);
        ...
        for (int i = 0; i < 2; i++) {
          float angle = random(TWO_PI);
          float dist = random(spaceshipSize * 0.2, spaceshipSize * 0.35);
          float x = cos(angle) * dist;
          float y = sin(angle) * dist;
          ellipse(x, y, random(12, 20), random(8, 15));
        }
      }
      
      // Critical damage at 1 heart left
      if (damageLevel >= 4) {
        // Major hull breach with sparks
        fill(255, 100, 0, 180);
        noStroke();
        for (int i = 0; i < 3; i++) {
          float angle = random(TWO_PI);
          float dist = random(spaceshipSize * 0.25, spaceshipSize * 0.4);
          float x = cos(angle) * dist + random(-5, 5);
          float y = sin(angle) * dist + random(-5, 5);
          ellipse(x, y, random(2, 6), random(2, 6));
        }
        
        // Electrical damage sparks
        stroke(255, 255, 100, 200);
        strokeWeight(2);
        for (int i = 0; i < 4; i++) {
          float angle = random(TWO_PI);
          float dist = spaceshipSize * 0.3;
          float x = cos(angle) * dist;
          float y = sin(angle) * dist;
          line(x, y, x + random(-8, 8), y + random(-8, 8));
        }
      }
    }
  ...
  if (currentAvatar > 0 && avatarImages[currentAvatar - 1] != null) {
    ...
    // Calculate avatar size (max 60 pixels tall)
    float avatarHeight = min(60, avatarImages[currentAvatar - 1].height);
...
void drawShrinkingSpaceship(float xPos, float yPos, float size) {
  ...
  float scaleFactor = size / 120.0; // Scale based on 120px UFO (actual size)
  ...
  // No avatar during black hole death animation
...
void drawLeaderboard() {
  // Draw background space objects first (behind text)
  drawBackgroundSpaceObjects();
...
void drawBackgroundSpaceObjects() {
  // Draw planets first (furthest back)
  drawPlanet(width * 0.82, height * 0.15, 90, frameCount * 0.008, color(150, 100, 200)); // Purple planet
  drawPlanet(width * 0.18, height * 0.75, 70, frameCount * 0.012, color(200, 150, 100)); // Orange planet
  
  // Draw satellite 1 (smaller, top-left corner)
  drawSatellite(width * 0.12, height * 0.15, 48, frameCount * 0.02);
  
  // Draw satellite 2 (larger, bottom-right corner)
  drawSatellite(width * 0.88, height * 0.85, 78, frameCount * 0.015);
  
  // Draw space shuttle (bottom-left area)
  drawSpaceShuttle(width * 0.12, height * 0.8, frameCount * 0.025);
}

void drawSatellite(float x, float y, float size, float wobbleTime) {
  pushMatrix();
  translate(x, y);
  
  // Wobble effect - gentle tilting back and forth
  float wobbleAngle = sin(wobbleTime) * 0.3; // ±0.3 radians (about ±17 degrees)
  rotate(wobbleAngle);
  ...
  stroke(100, 100, 150);
  strokeWeight(1);
  rect(-size * 0.7, 0, size * 0.4, size * 1.2); // Left panel
  rect(size * 0.7, 0, size * 0.4, size * 1.2);  // Right panel
  
  // Solar panel grid lines
  stroke(150, 150, 200);
  strokeWeight(1);
  for (int i = -2; i <= 2; i++) {
    line(-size * 0.9, i * size * 0.2, -size * 0.5, i * size * 0.2); // Left panel lines
    line(size * 0.5, i * size * 0.2, size * 0.9, i * size * 0.2);   // Right panel lines
  }
  for (int i = -1; i <= 1; i++) {
    line(-size * 0.7 + i * size * 0.15, -size * 0.6, -size * 0.7 + i * size * 0.15, size * 0.6); // Left vertical
    line(size * 0.7 + i * size * 0.15, -size * 0.6, size * 0.7 + i * size * 0.15, size * 0.6);   // Right vertical
  }
  ...
  stroke(150, 150, 150);
  strokeWeight(2);
  ellipse(0, -size * 0.15, size * 0.3, size * 0.3);
  
  // Antenna
  stroke(200, 200, 200);
  strokeWeight(3);
  line(0, -size * 0.5, 0, -size * 0.8);
  
  // Small blinking light
  if (frameCount % 120 < 60) { // Blink every 2 seconds
    fill(255, 100, 100);
    noStroke();
    ellipse(size * 0.25, size * 0.1, 4, 4);
  }
  
  popMatrix();
}

void drawSpaceShuttle(float x, float y, float wobbleTime) {
  pushMatrix();
  translate(x, y);
  
  // Wobble effect - gentle tilting back and forth
  float wobbleAngle = sin(wobbleTime) * 0.2; // ±0.2 radians (about ±11 degrees)
  rotate(wobbleAngle);
  
  float shuttleLength = 96; // 20% larger (was 80, now 96)
  float shuttleWidth = 30;  // 20% larger (was 25, now 30)
  ...
  // Fuselage (main body)
  beginShape();
  vertex(-shuttleLength * 0.4, 0); // Nose
  vertex(-shuttleLength * 0.2, -shuttleWidth * 0.3);
  vertex(shuttleLength * 0.4, -shuttleWidth * 0.3);
  vertex(shuttleLength * 0.4, shuttleWidth * 0.3);
  vertex(-shuttleLength * 0.2, shuttleWidth * 0.3);
  endShape(CLOSE);
  ...
  vertex(-shuttleLength * 0.1, shuttleWidth * 0.3);
  vertex(shuttleLength * 0.2, shuttleWidth * 0.3);
  vertex(shuttleLength * 0.3, shuttleWidth * 0.8);
  vertex(shuttleLength * 0.1, shuttleWidth * 0.8);
  endShape(CLOSE);
  ...
  fill(200, 200, 200);
  beginShape();
  vertex(shuttleLength * 0.2, 0);
  vertex(shuttleLength * 0.4, 0);
  vertex(shuttleLength * 0.45, -shuttleWidth * 0.6);
  vertex(shuttleLength * 0.3, -shuttleWidth * 0.6);
  endShape(CLOSE);
  ...
  stroke(80, 120, 160);
  strokeWeight(1);
  ellipse(-shuttleLength * 0.25, 0, shuttleWidth * 0.4, shuttleWidth * 0.3);
  
  // Engine nozzles
  fill(100, 100, 100);
  stroke(60, 60, 60);
  strokeWeight(2);
  ellipse(shuttleLength * 0.35, -shuttleWidth * 0.15, 8, 8);
  ellipse(shuttleLength * 0.35, 0, 8, 8);
  ellipse(shuttleLength * 0.35, shuttleWidth * 0.15, 8, 8);
  
  // Small navigation lights
  if (frameCount % 90 < 45) { // Blink every 1.5 seconds
    fill(100, 255, 100);
    noStroke();
    ellipse(shuttleLength * 0.1, shuttleWidth * 0.7, 3, 3); // Green wingtip
    fill(255, 100, 100);
    ellipse(shuttleLength * 0.1, -shuttleWidth * 0.7, 3, 3); // Red wingtip
  }
  
  popMatrix();
}

void drawPlanet(float x, float y, float size, float wobbleTime, color planetColor) {
  pushMatrix();
  translate(x, y);
  
  // Wobble effect - gentle tilting back and forth
  float wobbleAngle = sin(wobbleTime) * 0.15; // ±0.15 radians (about ±8 degrees)
  rotate(wobbleAngle);
  
  // Planet shadow (behind)
  fill(red(planetColor) * 0.3, green(planetColor) * 0.3, blue(planetColor) * 0.3);
  noStroke();
  ellipse(size * 0.1, size * 0.1, size, size);
  
  // Main planet body
  fill(planetColor);
  stroke(red(planetColor) * 0.7, green(planetColor) * 0.7, blue(planetColor) * 0.7);
  strokeWeight(2);
  ellipse(0, 0, size, size);
  
  // Planet surface details (darker patches)
  fill(red(planetColor) * 0.7, green(planetColor) * 0.7, blue(planetColor) * 0.7);
  noStroke();
  ellipse(-size * 0.2, -size * 0.1, size * 0.3, size * 0.2);
  ellipse(size * 0.15, size * 0.2, size * 0.25, size * 0.15);
  ellipse(-size * 0.1, size * 0.25, size * 0.2, size * 0.15);
  
  // Highlight (upper left)
  fill(red(planetColor) * 1.3, green(planetColor) * 1.3, blue(planetColor) * 1.3, 120);
  ellipse(-size * 0.15, -size * 0.15, size * 0.4, size * 0.3);
  
  // Atmospheric glow
  for (int i = 3; i >= 1; i--) {
    fill(red(planetColor), green(planetColor), blue(planetColor), 20);
    noStroke();
    ellipse(0, 0, size + i * 15, size + i * 15);
  }
