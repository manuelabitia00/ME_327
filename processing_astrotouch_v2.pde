...
// COMPATIBLE VERSION with Leader/Follower Arduino Setup
...
String serialPortName = "COM3"; // Change to your Arduino port (leader Arduino)
...
void updateSpaceshipPosition() {
  ...
  // Note: Arduino sends position with workspace offset already applied
  PVector targetPos = new PVector(
    map(pantoPos.x, ARD_X_MIN, ARD_X_MAX, 0, width),
    map(pantoPos.y, ARD_Y_MIN, ARD_Y_MAX, height, 0) // Y inverted for screen coords
  );
...
PVector calculateAsteroidForce(Asteroid ast) {
  ...
  float astY = map(ast.pos.y, 0, height, ARD_Y_MIN, ARD_Y_MAX);
...
PVector calculateBlackHoleForce(BlackHole bh) {
  ...
  float bhY = map(bh.pos.y, 0, height, ARD_Y_MIN, ARD_Y_MAX);
