...
// Original UFO graphics with added haptic feedback
...
String serialPortName = "COM3"; // Change to your Arduino port

// Pantograph workspace mapping (from Arduino code)
final float ARD_X_MIN = -80;  // mm
final float ARD_X_MAX = 80;   // mm  
final float ARD_Y_MIN = 110;  // mm
final float ARD_Y_MAX = 280;  // mm
...
PVector pantoPos = new PVector(0, 0); // Raw pantograph position in mm
...
// UFO variables (keeping original names for compatibility)
...
void initializeSerial() {
  try {
    ...
    println("Connected to Arduino on " + serialPortName);
  } catch (Exception e) {
...
void updateSpaceshipPosition() {
  // Read pantograph position from Arduino
  if (serialConnected) {
    readPantographPosition();
  } else {
    // Fallback to mouse for testing
    pantoPos.x = map(mouseX, 0, width, ARD_X_MIN, ARD_X_MAX);
    pantoPos.y = map(mouseY, 0, height, ARD_Y_MAX, ARD_Y_MIN);
  }
  
  // FIXED: For pantograph control, map pantograph position to screen coordinates
  PVector targetPos = new PVector(
    map(pantoPos.x, ARD_X_MIN, ARD_X_MAX, 0, width),
    map(pantoPos.y, ARD_Y_MIN, ARD_Y_MAX, height, 0)
  );
  ...
  pantoVelocity = PVector.sub(pantoPos, previousPantoPos);
  pantoVelocity.mult(60); // Convert to mm/s (assuming 60 FPS)
...
PVector calculateAsteroidForce(Asteroid ast) {
  ...
  float astY = map(ast.pos.y, 0, height, ARD_Y_MAX, ARD_Y_MIN);
  ...
  // Spring-like repulsion force (increases as UFO gets closer)
...
PVector calculateBlackHoleForce(BlackHole bh) {
  ...
  float bhY = map(bh.pos.y, 0, height, ARD_Y_MAX, ARD_Y_MIN);
...
PVector calculateMassDamping() {
  // Increased damping based on collected cargo
...
PVector calculateWallForces() {
  ...
  float wallStiffness = 1.5; // N/mm
  float wallMargin = 5.0; // mm
  
  // Check boundaries and apply repulsion forces
  if (pantoPos.x < ARD_X_MIN + wallMargin) {
    wallForce.x = wallStiffness * (ARD_X_MIN + wallMargin - pantoPos.x);
  }
  if (pantoPos.x > ARD_X_MAX - wallMargin) {
    wallForce.x = wallStiffness * (ARD_X_MAX - wallMargin - pantoPos.x);
  }
  if (pantoPos.y < ARD_Y_MIN + wallMargin) {
    wallForce.y = wallStiffness * (ARD_Y_MIN + wallMargin - pantoPos.y);
  }
  if (pantoPos.y > ARD_Y_MAX - wallMargin) {
    wallForce.y = wallStiffness * (ARD_Y_MAX - wallMargin - pantoPos.y);
  }
...
void sendForcesToArduino() {
  ...
  // Send force components to Arduino for motor torque calculation
...
void updateBlackHoles() {
  for (int i = blackHoles.size() - 1; i >= 0; i--) {
    ...
    if (bh.isOffScreen() || bh.shouldRemove()) {
      ...
      // Apply much stronger gravitational effects (visual only - haptic is calculated separately)
...
void drawUI() {
  ...
  textSize(16);
  textAlign(LEFT);
  text("Cargo: " + cargoCollected, 20, height - 100);
  text("Mass: " + nf(virtualMass, 1, 1) + "x", 20, height - 80);
  text("Speed: " + nf(gameSpeedMultiplier, 1, 1) + "x", 20, height - 60);
  text("Pantograph Control Active", 20, height - 40);
  
  // Pantograph position display
  fill(150, 150, 255);
  textSize(12);
  text("Panto Pos: (" + nf(pantoPos.x, 1, 1) + ", " + nf(pantoPos.y, 1, 1) + ") mm", 20, height - 20);
}
...
void restartGame() {
  ...
  currentForce.set(0, 0); // Reset forces
  asteroids.clear();
  cargoItems.clear();
  particles.clear();
  blackHoles.clear();
