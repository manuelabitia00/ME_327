...
// Debug flag
final boolean DEBUG_MODE    = false; // Changed to false to remove red dot
...
// IMPROVED HAPTIC FEEDBACK CONSTANTS - Much more conservative and effective
// Motor stall torque is 51.975 mN·m, so we limit forces to protect the motor

// Black hole haptic constants - Gradual pull that builds up slowly when close
float BLACK_HOLE_FORCE_MAX = 0.2;         // N - Slightly increased max force but very gradual
float BLACK_HOLE_RADIUS_MM = 60.0;         // mm - Smaller radius, more focused effect
float BLACK_HOLE_CLOSE_THRESHOLD = 25.0;   // mm - Distance when pull starts to feel strong
float BLACK_HOLE_GRADUAL_FACTOR = 0.1;     // Controls how gradually the pull builds up

// Asteroid impact haptic constants - Simple impulse implementation
float ASTEROID_IMPACT_FORCE_MAX = 0.1;    // N - Maximum impact force (brief impulse)
float ASTEROID_IMPACT_DECAY_RATE = 0.85;   // How quickly impact force decays each frame
int ASTEROID_IMPACT_MIN_FRAMES = 15;       // Minimum frames for impact feedback

// Cargo mass haptic constants - SIMPLIFIED for stability
float CARGO_DAMPING_COEFFICIENT = 0.002;   // N·s/mm - Simple linear damping per cargo item
float CARGO_MAX_DAMPING = 0.012;           // N·s/mm - Maximum total damping to prevent instability

// Haptic feedback state variables - SIMPLIFIED for better performance
PVector asteroidImpactForce = new PVector(0, 0);
float asteroidImpactMagnitude = 0;
int asteroidImpactFramesRemaining = 0;

// Previous position for velocity calculation (needed for cargo damping)
float prevXPos_mm_haptic = 0;
float prevYPos_mm_haptic = 0;
...
void draw()
{
   ...
   if (showPressStart) {
     ...
     updateHapticFeedback(); // Update haptic feedback states
     checkCollisions();
...
// IMPROVED HAPTIC FEEDBACK UPDATE - Much simpler and more effective
void updateHapticFeedback() {
  // Update asteroid impact - simple exponential decay
  if (asteroidImpactFramesRemaining > 0) {
    asteroidImpactFramesRemaining--;
    asteroidImpactMagnitude *= ASTEROID_IMPACT_DECAY_RATE;
    
    // Apply force in the stored direction
    asteroidImpactForce.mult(ASTEROID_IMPACT_DECAY_RATE);
  } else {
    // Reset impact force when timer expires
    asteroidImpactForce.set(0, 0);
    asteroidImpactMagnitude = 0;
  }
}
   ...
   if (in.startsWith("{") && in.endsWith("}"))
   {
      ...
      if (parts.length == 2)
      {
         try
         {
            ...
            // Store previous position for velocity calculation in haptic feedback
            prevXPos_mm_haptic = xPos_mm;
            prevYPos_mm_haptic = yPos_mm;
...
// IMPROVED FORCE CALCULATION - Much better haptic rendering with simple, effective formulas
float[] getForces(float xPos_mm, float yPos_mm)
{ 
  PVector force = new PVector(0.0, 0.0);
  
  // 1. WALL FORCES - Keep the original working implementation
  if (xPos_mm < X_MIN_MM)
  {
  ...
  // Only add game haptic forces if game is running
  if (gameRunning && spaceshipPos != null) {
    
    // 2. BLACK HOLE FORCES - IMPROVED gradual pull that builds slowly when close
    for (BlackHole bh : blackHoles) {
      // Convert black hole game coordinates to real-world mm coordinates
      float bhX_mm = (bh.pos.x * (X_MAX_MM - X_MIN_MM) / width) - X_MAX_MM;
      float bhY_mm = (bh.pos.y * (Y_MAX_MM - Y_MIN_MM) / height) + Y_MIN_MM;
      
      // Calculate distance in mm
      float dx_mm = bhX_mm - xPos_mm;
      float dy_mm = bhY_mm - yPos_mm;
      float distance_mm = sqrt(dx_mm * dx_mm + dy_mm * dy_mm);
      
      // GRADUAL PULL: Only feel it when getting close, builds up slowly
      if (distance_mm < BLACK_HOLE_RADIUS_MM && distance_mm > 3.0) {
        
        float forceMagnitude = 0;
        
        // Very gentle pull at medium distance
        if (distance_mm > BLACK_HOLE_CLOSE_THRESHOLD) {
          // Barely noticeable at medium distance
          float mediumRange = BLACK_HOLE_RADIUS_MM - BLACK_HOLE_CLOSE_THRESHOLD;
          float mediumFactor = (BLACK_HOLE_RADIUS_MM - distance_mm) / mediumRange;
          forceMagnitude = BLACK_HOLE_FORCE_MAX * 0.1 * mediumFactor; // Only 10% of max force
        } 
        // Gradually increasing pull when getting close
        else {
          // Build up gradually as you get very close
          float closeFactor = (BLACK_HOLE_CLOSE_THRESHOLD - distance_mm) / BLACK_HOLE_CLOSE_THRESHOLD;
          // Use squared factor for more gradual buildup: force increases slowly then rapidly
          forceMagnitude = BLACK_HOLE_FORCE_MAX * closeFactor * closeFactor * BLACK_HOLE_GRADUAL_FACTOR;
        }
        
        // Apply smooth gradual force
        if (forceMagnitude > 0) {
          // Normalize direction vector
          float dirX = dx_mm / distance_mm;
          float dirY = dy_mm / distance_mm;
          
          // Apply the gradual attraction force
          force.x += forceMagnitude * dirX;
          force.y += forceMagnitude * dirY;
          
          if (DEBUG_MODE) {
            println("Black hole force: " + nf(forceMagnitude, 1, 4) + " at distance: " + nf(distance_mm, 1, 1));
          }
        }
      }
    }
    
    // 3. ASTEROID IMPACT FORCES - Simple impulse that decays over time
    force.x += asteroidImpactForce.x;
    force.y += asteroidImpactForce.y;
    
    // 4. SIMPLE CARGO DAMPING - Basic linear damping that increases with cargo count
    if (cargoCollected > 0) {
      // Calculate current velocity in mm/frame
      float currentVelX_mm = (xPos_mm - prevXPos_mm_haptic);
      float currentVelY_mm = (yPos_mm - prevYPos_mm_haptic);
      
      // SIMPLE LINEAR DAMPING: F = -b * v
      // More cargo = more damping (feels heavier to move)
      float totalDamping = cargoCollected * CARGO_DAMPING_COEFFICIENT;
      
      // Clamp damping to prevent instability
      totalDamping = constrain(totalDamping, 0, CARGO_MAX_DAMPING);
      
      // Apply simple velocity-proportional damping force
      force.x -= totalDamping * currentVelX_mm * TARGET_FRAME_RATE;
      force.y -= totalDamping * currentVelY_mm * TARGET_FRAME_RATE;
      
      if (DEBUG_MODE && cargoCollected > 0) {
        float speed = sqrt(currentVelX_mm * currentVelX_mm + currentVelY_mm * currentVelY_mm);
        println("Cargo damping: " + nf(totalDamping, 1, 4) + " for " + cargoCollected + " items, speed: " + nf(speed, 1, 2));
      }
    }
  }

  // CRITICAL: Conservative force limiting to protect the motor
  // REDUCED maximum forces for gentler feel while maintaining safety
  force.x = constrain(force.x, -0.15, 0.15); // Reduced from 0.2 to 0.15N
  force.y = constrain(force.y, -0.15, 0.15);

  return new float[] {force.x, force.y};
}

// IMPROVED asteroid impact haptic feedback - Simple impulse with decay
void triggerAsteroidImpact(PVector impactDirection) {
  // Convert impact direction from screen coordinates to haptic coordinates
  PVector hapticDirection = new PVector();
  hapticDirection.x = impactDirection.x * (X_MAX_MM - X_MIN_MM) / width;
  hapticDirection.y = impactDirection.y * (Y_MAX_MM - Y_MIN_MM) / height;
  hapticDirection.normalize();
  
  // Set impact force as a simple impulse
  asteroidImpactForce.set(hapticDirection.x * ASTEROID_IMPACT_FORCE_MAX, 
                          hapticDirection.y * ASTEROID_IMPACT_FORCE_MAX);
  asteroidImpactMagnitude = ASTEROID_IMPACT_FORCE_MAX;
  asteroidImpactFramesRemaining = ASTEROID_IMPACT_MIN_FRAMES;
  
  if (DEBUG_MODE) {
    println("Asteroid impact triggered: force=" + ASTEROID_IMPACT_FORCE_MAX);
  }
}
...
float[] getTorques(float Fx_N, float Fy_N, float P2x, float P2y, float P4x, float P4y, float P24norm, float P2Hnorm, float P3Hnorm, float theta1Rad, float theta5Rad)
{
  ...
  // CRITICAL: Very conservative torque limiting for motor protection
  // Motor stall torque is 51.975 mN·m, limit to 50% for safety = 25 mN·m
  float MAX_TORQUE = 25.0; // mN·m (50% of stall torque for excellent safety margin)
...
void checkCollisions() {
  ...
  for (int j = asteroids.size() - 1; j >= 0; j--) {
    ...
    if (PVector.dist(spaceshipPos, ast.pos) < (spaceshipSize/2 + ast.size/2)) {
      ...
      // Calculate impact direction for haptic feedback
      PVector impactDir = PVector.sub(spaceshipPos, ast.pos);
      impactDir.normalize();
      triggerAsteroidImpact(impactDir);
...
void restartGame() {
  ...
  // Reset haptic feedback variables
  asteroidImpactForce.set(0, 0);
  asteroidImpactMagnitude = 0;
  asteroidImpactFramesRemaining = 0;
