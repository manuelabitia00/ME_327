...
String serialPortName = "COM7"; // Change to your Arduino port

// Pantograph workspace mapping (from Arduino code) - CORRECTED
...
void initializeSerial() {
  try {
    ...
    arduinoPort = new Serial(this, serialPortName, 115200); // CORRECTED BAUD RATE
...
void readPantographPosition() {
  if (arduinoPort.available() > 0) {
    ...
    if (data != null) {
      data = data.trim();
      String[] values = data.split(" "); // CORRECTED: space-separated format
      if (values.length == 2) {
        try {
...
void sendForcesToArduino() {
  ...
  // Send force components to Arduino (space-separated format)
...
void drawDebugInfo() {
  ...
  textAlign(LEFT);
  textSize(12);
  text("Pantograph Debug Info:", 10, height - 140);
  text("Position: (" + nf(pantoPos.x, 1, 1) + ", " + nf(pantoPos.y, 1, 1) + ") mm", 10, height - 125);
  text("Velocity: (" + nf(pantoVelocity.x, 1, 1) + ", " + nf(pantoVelocity.y, 1, 1) + ") mm/s", 10, height - 110);
  text("Force: (" + nf(currentForce.x, 1, 2) + ", " + nf(currentForce.y, 1, 2) + ") N", 10, height - 95);
  text("Virtual Mass: " + nf(virtualMass, 1, 2), 10, height - 80);
  text("Serial: " + (serialConnected ? "Connected" : "Disconnected"), 10, height - 65);
}
